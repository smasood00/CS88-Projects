...
CREATE TABLE long_album as
  SELECT a.title FROM albums as a, tracks as t WHERE t.albumId = a.albumId AND t.milliseconds > 360000;
...
CREATE TABLE album_count as
  SELECT ar.name, COUNT(ar.name) FROM artists as ar, albums as al WHERE ar.artistId = al.artistId GROUP BY ar.name;


-- Question 8
CREATE TABLE busiest_artists as
  SELECT ar.name, COUNT(ar.name) FROM artists as ar, albums as al, tracks as t WHERE al.albumId = t.albumId AND ar.artistId = al.artistId GROUP BY ar.name;
