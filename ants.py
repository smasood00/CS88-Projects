...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        short_count = 0
        if self.min_range == 0:
            while current != beehive and (len(current.bees) == 0) and short_count >= self.min_range and short_count < self.max_range:
        ...
        long_count = 0
        if self.min_range > 0:
            while long_count < self.min_range:
                current = current.entrance
                long_count = long_count + 1

            while current != beehive and (len(current.bees) == 0):
                current = current.entrance
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while previous:
                ant = previous.ant
                if isinstance(ant, BodyguardAnt) and ant.contained_ant and ant.contained_ant not in self.doubled:
...
class AntColony(object):
   ...
   """ Everything below is not my work, it was already given from the instructors """
